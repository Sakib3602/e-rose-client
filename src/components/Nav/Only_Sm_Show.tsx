...
export default function Only_Sm_Show() {
  ...
  const updateWishlistCount = () => {
    ...
    const currentWishlist: WishlistItem[] = storedWishlist
  ...
  const handleRemoveItem = (itemId: string) => {
    const updatedCart = cartD.filter((item) => {
      ...
      console.log(
        `Comparing item.id: "${itemIdStr}" with itemId: "${compareId}" → keep: ${shouldKeep}`
      );
      return shouldKeep;
    });
  ...
  return (
    <>
              ...
              if (item.id === "cart") {
                return (
                  <div
                    ...
                    onClick={() =>
                      handleNavClick(item.id, item.href, item.label)
                    }
                  >
                    {content}
                  </div>
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetContent
          side="right"
          className="w-[280px] sm:w-[320px] border-l border-border/40 flex flex-col"
        >
