...
const ProductForm: React.FC = () => {
  ...
  const handlePic1Upload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPic1(file)
    setFormData((prevData) => ({ ...prevData, pic1: file }))
  }

  const handlePic2Upload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0] || null
    setPic2(file)
    setFormData((prevData) => ({ ...prevData, pic2: file }))
  }
  ...
  try {
    ...
    if (pic1) {
      ...
      const response1 = await axios.post(
        `https://api.cloudinary.com/v1_1/${import.meta.env.VITE_HOST}/image/upload`,
        imageData1
      )
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {sizes.map((size) => (
                <button
                  ...
                  style={selectedSizes.includes(size) ? { backgroundColor: "#761A24" } : {}}
                >
                  {size}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-gray-400 transition-colors">
                <input type="file" accept="image/*" onChange={handlePic1Upload} className="hidden" id="pic1-upload" />
            ...
            <div className="space-y-3">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-gray-400 transition-colors">
                <input type="file" accept="image/*" onChange={handlePic2Upload} className="hidden" id="pic2-upload" />
          ...
          <div className="flex justify-end space-x-4 pt-6 border-t">
            ...
            <Button onClick={handleSubmit} className="text-white" style={{ backgroundColor: "#761A24" }}>
              Add Product
            </Button>
          </div>
        </CardContent>
